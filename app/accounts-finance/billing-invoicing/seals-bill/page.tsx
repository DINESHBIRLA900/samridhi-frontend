...
export default function SealsBillPage() {
    ...
    const fetchCustomers = async () => {
        try {
            // Map 'Dealer' -> 'B2B', 'Farmer' -> 'B2C'
            const apiType = partnerType === 'Dealer' ? 'B2B' : 'B2C';
            const data = await getCustomers(apiType);
            // Note: getCustomers signature: (type?: string, search?: string)
            // Assuming getCustomers returns array directly or inside data property based on service file
            // Service file: return response.data; containing array? Or { customers: [] }?
            // Usually response.data is the payload. Let's assume array or handle both.
    ...
    const handleTypeChange = (type: 'Dealer' | 'Farmer') => {
        ...
        setCustomerMobile("");
        setCustomerAddress("");
        // Trigger fetch next render via effect or imperative?
        // Let's use clean approach: we need to re-fetch when type changes.
        // But setState is async.
        // We can call fetch with new type immediately.
        ...
        <div className="w-full max-w-[1600px] mx-auto">
            ...
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-gray-800">Seals Bill</h1>
                        <p className="text-gray-600 text-sm">Create new sales invoice</p>
                    </div>
                </div>
